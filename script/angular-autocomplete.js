  ...
  return {
    ...
    template: '\
        <div class="autocomplete {{ attrs.class }}" id="{{ attrs.id }}">\
          ...
          <ul ng-if="!noAutoSort" ng-show="searchFilter.length>0&&completing && (suggestions | filter:searchFilter).length > 0">\
            <li\
              suggestion\
          ...
          <ul ng-if="noAutoSort" ng-show="searchFilter.length>0&&completing && (suggestions | filter:searchFilter).length > 0">\
            <li\
              suggestion\
              ng-repeat="suggestion in suggestions | filter:{name:searchFilter} track by $index"\
              index="{{ $index }}"\
              val="{{ suggestion.name }}"\
              ng-class="{ active: ($index === selectedIndex) }"\
              ng-click="select(suggestion)"\
...
app.filter('highlight', ['$sce', function ($sce) {
  return function (obj, searchParam) {
	  var input='';
    ...
    if (searchParam) {
      ...
      if (words.length) {
        input = obj.name.replace(exp, "<span class=\"highlight\">$1</span>");
      }
    }
